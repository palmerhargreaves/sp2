...
class DealersStatisticsCalculateByGeneral {
                    ...
                    if (!empty($this->_quarter)) {
                        if ($this->_quarter == $model['model_q']) {
                            ...
                            if ($this->_extended_category_info) {
                                $this->_statistic_aggregator[$model[$this->_category_or_type_id]][$model['model_type_id']][$model['activity_id']] = 0;
                            } else {
                                $this->_statistic_aggregator[$model[$this->_category_or_type_id]][$model['activity_id']] = 0;
                            }
                        }
                    } else {
                        $models_list_by_quarter[$temp_models[$completed_model['object_id']]['dealer_id']][] = $model;
                        ...
                        if (!empty($this->_quarter)) {
                            if ($model['model_q'] == $this->_quarter) {
                                if ($this->_data_type == self::DATA_TYPE_COUNTS) {
                                    //Информация в разрезе категории
                                    if ($this->_extended_category_info) {
                                        $this->_statistic_aggregator[$model[$this->_category_or_type_id]][$model['model_type_id']][$model['activity_id']]++;
                                    } else {
                                        $this->_statistic_aggregator[$model[$this->_category_or_type_id]][$model['activity_id']]++;
                                    }
                                } else {
                                    //Информация в разрезе категории
                                    if ($this->_extended_category_info) {
                                        $this->_statistic_aggregator[$model[$this->_category_or_type_id]][$model['model_type_id']][$model['activity_id']] += $model['cost'];
                                    } else {
                                        $this->_statistic_aggregator[$model[$this->_category_or_type_id]][$model['activity_id']] += $model['cost'];
                                    }
                                }
                            }
                        } else {
    ...
    public function getData() {
        ...
        foreach ($available_activities as $activity_id => $activity_data) {
            ...
            $aSheet->setCellValueByColumnAndRow($column++, $row, $activity_data.'('.$activity_id.')');
        ...
        if ($this->_extended_category_info && $this->isCategoryType()) {
            ...
            foreach ($available_types as $category_id => $category_name) {
                ...
                foreach ($available_category_types[$category_id] as $type_id => $type_name) {
                    ...
                    foreach ($this->_statistic_aggregator[$category_id][$type_id] as $activity_id => $data) {
            ...
            foreach ($available_types as $type_id => $type_name) {
                ...
                foreach ($this->_statistic_aggregator[$type_id] as $activity_id => $data) {
                    if (array_key_exists($activity_id, $activities_columns_schema)) {
                        $aSheet->setCellValueByColumnAndRow($activities_columns_schema[$activity_id], $row, $data);
                        $aSheet->getStyle($activities_columns_schema[$activity_id] . $row)
                            ->getNumberFormat()
        ...
        $file_name = '/uploads/statistics_general.xlsx';

        //$objWriter = new PHPExcel_Writer_Excel5($pExcel);
        $objWriter = PHPExcel_IOFactory::createWriter($pExcel, "Excel2007");
        $objWriter->save(sfConfig::get('sf_root_dir') . '/www/uploads/statistics_general.xlsx');
